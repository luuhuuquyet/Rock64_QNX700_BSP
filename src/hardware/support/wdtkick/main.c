...
int main(int argc, char *argv[])
{
    ...
    uintptr_t   base;
    paddr_t     physbase = RK3328_WDT_BASE;
    int         verbose = 0;
    ...
    while (1) {
        out32(base + RK3328_WDT_CRR, 0x76);     // reset counter
        delay(time);
    }

done:
    munmap_device_io(base, 0x14);
